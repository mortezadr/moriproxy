...
void ConnectionsManager::setProxySettings(std::string address, uint16_t port, std::string username, std::string password, std::string secret, int32_t type) {
    scheduleTask([&, address, port, username, password, secret, type] {
        std::string newSecret;
        if (type == 2 || type == 3) {
            newSecret = secret;
        } else {
            newSecret = decodeSecret(secret);
        }
        bool secretChanged = proxySecret != newSecret;
        bool typeChanged = proxyType != type;
        bool reconnect = proxyAddress != address || proxyPort != port || username != proxyUser || proxyPassword != password || secretChanged || typeChanged;
...
int64_t ConnectionsManager::checkProxy(std::string address, uint16_t port, std::string username, std::string password, std::string secret, int32_t type, onRequestTimeFunc requestTimeFunc, jobject ptr1) {
...
void ConnectionsManager::checkProxyInternal(ProxyCheckInfo *proxyCheckInfo) {
    ...
    if (freeConnectionNum == -1) {
        ...
        if (connection != nullptr) {
            connection->setOverrideProxy(proxyCheckInfo->address, proxyCheckInfo->port, proxyCheckInfo->username, proxyCheckInfo->password, proxyCheckInfo->secret, proxyCheckInfo->proxyType);
            connection->suspendConnection();
