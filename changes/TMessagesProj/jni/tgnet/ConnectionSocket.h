...
public:
    ...
    bool isDisconnected();
    void dropConnection();
    void setOverrideProxy(std::string address, uint16_t port, std::string username, std::string password, std::string secret, int32_t type = 0);
...
private:
    ...
    // Worker Proxy (TWP) support
    SSL *ssl = nullptr;
    std::vector<uint8_t> workerInputBuffer;
    std::string webSocketKey;
    std::string workerHost;
    bool isWorker = false;

    void sendWebSocketUpgradeRequest();
    bool processWebSocketUpgradeResponse();
    bool processWebSocketIncomingData();
    void writeWebSocketBinaryFrame(const uint8_t *data, size_t len);
    void sendWebSocketPong(const uint8_t *data, size_t len);
