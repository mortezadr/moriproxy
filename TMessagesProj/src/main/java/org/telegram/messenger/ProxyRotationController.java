...
public class ProxyRotationController implements NotificationCenter.NotificationCenterDelegate {
    ...
    private Runnable checkProxyAndSwitchRunnable = () -> {
        ...
        for (int i = 0; i < SharedConfig.proxyList.size(); i++) {
            ...
            proxyInfo.proxyCheckPingId = ConnectionsManager.getInstance(currentAccount).checkProxy(proxyInfo.address, proxyInfo.port, proxyInfo.username, proxyInfo.password, proxyInfo.secret, proxyInfo.type, time -> AndroidUtilities.runOnUIThread(() -> {
    ...
    private void switchToAvailable() {
        ...
        for (SharedConfig.ProxyInfo info : sortedList) {
            ...
            editor.putInt("proxy_type", info.type);
            ...
            ConnectionsManager.setProxySettings(true, SharedConfig.currentProxy.address, SharedConfig.currentProxy.port, SharedConfig.currentProxy.username, SharedConfig.currentProxy.password, SharedConfig.currentProxy.secret, SharedConfig.currentProxy.type);
            break;
        }
    }
